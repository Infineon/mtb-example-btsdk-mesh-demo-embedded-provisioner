...
1. Build and download the application to several AIROC&#8482; Kits.
2. Push and release a button on one of the kits to set it up as an embedded provisioner.
3. For 20721B2 based AIROC&#8482; kits with the audio shield equipped, use the Custom button on audio shield board as application button rather than the button on the base board.

Note: The CYW920721M2EVK-01 kit includes the audio shield, but does not connect buttons or LEDs.  To use the Embedded Provisioner or Dimmer Self Config apps with this kit, remove the audio shield, and flywire J3.6 (D12) to J11.3 (LED2) and J4.1 (D7) to J12.1 (USER button).  See makefile AUDIO\_SHIELD\_REMOVED flag.
...
4. For 20835B1 based AIROC&#8482; kits, Mesh debug trace flags (MESH\_MODELS\_DEBUG\_TRACES, MESH\_CORE\_DEBUG\_TRACES, and MESH\_PROVISIONER\_DEBUG\_TRACES) shall all be set to 0 in the makefile or on the make command line.
